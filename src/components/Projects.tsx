  ...
  const projects = [
    {
      ...
      year: "Third Year",
      role: "Team Associate",
      description: "Developed a machine learning application for breast cancer detection using multiple classification algorithms. Gained expertise in data preprocessing, feature engineering, and model optimization.",
      technologies: ["Python", "Machine Learning", "SVM", "Decision Tree", "KNN", "Naive Bayes", "Flask"],
      highlights: [
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, index) => (
            <Card 
              key={index}
              className="group hover:shadow-large transition-smooth border-2 hover:border-primary/50 animate-fade-in-up overflow-hidden"
              style={{ animationDelay: `${index * 150}ms` }}
            >
              <CardHeader>
                {/* Icon with gradient background */}
                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-primary/20 to-secondary/20 group-hover:from-primary/30 group-hover:to-secondary/30 transition-smooth flex items-center justify-center mb-4">
                  <project.icon className="w-7 h-7 text-primary" />
                </div>
                
                <div className="flex items-center justify-between mb-2">
                  <Badge variant="secondary">{project.year}</Badge>
                  <Badge variant="outline" className="border-primary/30">{project.role}</Badge>
                </div>
